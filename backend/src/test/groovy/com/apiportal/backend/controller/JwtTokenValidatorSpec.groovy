...
@Subject(com.apiportal.backend.infra.security.JwtTokenValidator)
class JwtTokenValidatorSpec extends Specification {

    private com.apiportal.backend.infra.security.JwtTokenValidator validator
    private JwkProvider jwkProvider

    def setup() {
        jwkProvider = Stub(com.apiportal.backend.infra.security.config.KeycloakJwkProvider)
        validator = new com.apiportal.backend.infra.security.JwtTokenValidator(jwkProvider)
    }
    ...
    def "Create AccessToken from String value"() {
        ...
        def accessToken = validator.validateAuthorizationHeader(com.apiportal.backend.infra.security.AccessToken.BEARER + token)
    ...
    def "AccessToken with incorrect signature"() {
        ...
        when: "Validate access token"
        validator.validateAuthorizationHeader(com.apiportal.backend.infra.security.AccessToken.BEARER + token)

        then: "Token has invalid signature"
        def exception = thrown(com.apiportal.backend.infra.security.InvalidTokenException)
